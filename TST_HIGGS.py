...
parser.add_argument('--n', type=int, default=5000, help='number of data in each set')
...
parser.add_argument('--lr_RoD', type=float, default=0.00005, help='learning rate for MMD-RoD')
...
def train_ME(s1, s2):
    ME_test = ME(HD=False, hyperparameters=(alpha, 1,1,10,15))
    ME_test.train(s1, s2)
    return ME_test

def train_SCF(s1, s2):
    SCF_test = SCF(HD=False, hyperparameters=(alpha, 1,1,10,15))
