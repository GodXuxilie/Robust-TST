...
import argparse
import os
from TST_utils import MatConvert, MMDu, TST_MMD_adaptive_bandwidth, TST_LCE, TST_ME, TST_SCF, TST_C2ST, C2ST_NN_fit,MMDu, TST_MMD_u, TST_MMD_adaptive_WB, TST_WBMMD_u
...
parser.add_argument('--weight', type=str, default='3,45,4,3,40,3', help='attack weight')
...
out_dir = 'results/HIGGS_'
if args.robust_kernel:
    out_dir += 'robust_'
out_dir += str(args.num_steps) + '_' + str(args.n) + '_' + str(weight)

if not os.path.exists(out_dir):
        os.makedirs(out_dir)

# Load data
data = pickle.load(open('{}/HIGGS_TST.pckl'.format(args.data_dir), 'rb'))
if not args.type1:
    dataX = data[0]
    dataY = data[1]
else:
...
def train_MMD_RoD(s1, s2):
    ...
    sigmaOPT_RoD = MatConvert(np.ones(1) * np.sqrt(2*d), device, dtype)  # d = 3,5 ??
    sigmaOPT_RoD.requires_grad = True
    sigma0OPT_RoD = MatConvert(np.ones(1) * np.sqrt(0.005), device, dtype)
    sigma0OPT_RoD.requires_grad = False
    optimizer_u_RoD = torch.optim.Adam(list(model_u_RoD.parameters()) + [epsilonOPT_RoD] + [sigmaOPT_RoD] + [sigma0OPT_RoD], lr=0.00005)

    for t in range(N_epoch):
        ep_RoD = torch.exp(epsilonOPT_RoD) / (1 + torch.exp(epsilonOPT_RoD))  # 10 ** (-10)#
        ...
        TSTAttack = two_sample_test_attack(num_steps=args.attack_num, epsilon=args.epsilon, step_size=args.step_size, dynamic_eta=args.dynamic_eta, 
                                            ...
                                            max_scale=dataY.max(), min_scale=dataY.min())
        adv_Fake_imgs = TSTAttack.attack(torch.Tensor(s1).cuda(), torch.Tensor(s2).cuda(), torch.Tensor(nat_s2).cuda(), weight=[0,0,0,0,0,0,1])
        S = np.concatenate((s1, adv_Fake_imgs.cpu().numpy()), axis=0)
        S = MatConvert(S, device, dtype)
        modelu_output = model_u_RoD(S)
        TEMP = MMDu(modelu_output, n, S, sigma_RoD, sigma0_u_RoD, ep_RoD)
        mmd_value_temp = -1 * (TEMP[0]+10**(-8))  # 10**(-8)
        mmd_std_temp = torch.sqrt(TEMP[1]+10**(-8))  # 0.1
        STAT_u = torch.div(mmd_value_temp, mmd_std_temp)
        ...
        if args.BA:
            ...
            TEMP_nat = MMDu(modelu_output_nat, n, S_nat, sigma_RoD, sigma0_u_RoD, ep_RoD)
            mmd_value_temp_nat = -1 * (TEMP_nat[0]+10**(-8))  # 10**(-8)
            mmd_std_temp_nat = torch.sqrt(TEMP_nat[1]+10**(-8))  # 0.1
...
def train_C2ST_L(s1, s2):
    S = np.concatenate((s1, s2), axis=0)
    S = MatConvert(S, device, dtype)
    y = (torch.cat((torch.zeros(n, 1), torch.ones(n, 1)), 0)).squeeze(1).to(device, dtype).long()
    pred, STAT_C2ST_L, model_C2ST_L, w_C2ST_L, b_C2ST_L = C2ST_NN_fit(S, y, n, x_in, H, x_out, 0.001,
                                                                      2000, batch_size, device, dtype)
...
def train_C2ST_S(s1, s2):
    ...
    S = MatConvert(S, device, dtype)
    y = (torch.cat((torch.zeros(n, 1), torch.ones(n, 1)), 0)).squeeze(1).to(device, dtype).long()
    pred, STAT_C2ST_S, model_C2ST_S, w_C2ST_S, b_C2ST_S = C2ST_NN_fit(S, y, n, x_in, H, x_out, 0.001, 2000,
...
def train_MMD_G(s1, s2):
    ...
    for t in range(2000):
        TEMPa = MMDu(S, n, S, 0, sigma0, is_smooth=False)
    ...
    # h_adaptive, threshold_adaptive, mmd_value_adaptive = TST_MMD_adaptive_bandwidth(S, N_per, n, S, 0, sigma0, alpha, device, dtype)
...
def train_ME(s1, s2):
    S = np.concatenate((s1, s2), axis=0)
    S = MatConvert(S, device, dtype)
    test_locs_ME, gwidth_ME = TST_ME(S, n, alpha, is_train=True, test_locs=1, gwidth=1, J=10, seed=15)
    # h_ME = TST_ME(S, n, alpha, is_train=False, test_locs=test_locs_ME, gwidth=gwidth_ME, J=10, seed=15)
    return test_locs_ME, gwidth_ME

def train_SCF(s1, s2):
    S = np.concatenate((s1, s2), axis=0)
    S = MatConvert(S, device, dtype)
    test_freqs_SCF, gwidth_SCF = TST_SCF(S, n, alpha, is_train=True, test_freqs=1, gwidth=1, J=10, seed=15)
    # h_SCF = TST_SCF(S, n, alpha, is_train=False, test_freqs=test_freqs_SCF, gwidth=gwidth_SCF, J=10, seed=15)
...
def test_procedure(S, MMD_D_args, MMD_G_args, ME_args, SCF_args, C2ST_S_args, C2ST_L_args, MMD_RoD_args=None):
    if args.WB:
        # MMD-D
        h_D, _, _ = TST_WBMMD_u(MMD_D_args[0](S), N_per, n, S, MMD_D_args[1], MMD_D_args[2], MMD_D_args[3], alpha, device, dtype, args.ln)
        # MMD-G
        h_G, _, _ = TST_MMD_adaptive_WB(S, N_per, n, 0, MMD_G_args, alpha, device, dtype, args.ln)
    else:
        # MMD-D
        h_D, _, _ = TST_MMD_u(MMD_D_args[0](S), N_per, n, S, MMD_D_args[1], MMD_D_args[2], MMD_D_args[3], alpha, device, dtype)
        # MMD-G
        h_G, _, _ = TST_MMD_adaptive_bandwidth(S, N_per, n, S, sigma, MMD_G_args, alpha, device, dtype)
    if args.robust_kernel:
        if args.WB:
            h_RoD, _, _ = TST_WBMMD_u(MMD_RoD_args[0](S), N_per, n, S, MMD_RoD_args[1], MMD_RoD_args[2], MMD_RoD_args[3], alpha, device, dtype, args.ln)
        else:
            h_RoD, _, _ = TST_MMD_u(MMD_RoD_args[0](S), N_per, n, S, MMD_RoD_args[1], MMD_RoD_args[2], MMD_RoD_args[3], alpha, device, dtype)
    else:
        h_RoD = 0
    # ME
    h_ME = TST_ME(S, n, alpha, is_train=False, test_locs=ME_args[0], gwidth=ME_args[1], J=5, seed=15)
    # SCF
    h_SCF = TST_SCF(S, n, alpha, is_train=False, test_freqs=SCF_args[0], gwidth=SCF_args[1], J=5, seed=15)
    # C2ST-S
    h_C2ST_S, _, _ = TST_C2ST(S, n, N_per, alpha, C2ST_S_args[0], C2ST_S_args[1], C2ST_S_args[2], device, dtype)
    # C2ST-L
    h_C2ST_L, _,_ = TST_LCE(S, n, N_per, alpha, C2ST_L_args[0], C2ST_L_args[1], C2ST_L_args[2], device, dtype)
...
for kk in range(args.trails):
    # Setup random seed for sampling data
    torch.manual_seed(seed1+ kk * 19 + n)
    torch.cuda.manual_seed(seed1 + kk * 19 + n)
    # Generate Higgs (P,Q)
    np.random.seed(seed=seed1 * kk + n)
    ...
    for k in range(N):
        ...
        ind1 = np.random.choice(N1_T, n, replace=False)
        np.random.seed(seed=seed2 * (k+2) + n)
        ind2 = np.random.choice(N2_T, n, replace=False)
        s1 = dataX[ind1, :4]
        s2 = dataY[ind2, :4]
        S = np.concatenate((s1, s2), axis=0)
    ...
    if args.surrogate:
        ...
        # Setup random seed for sampling data
        torch.manual_seed(seed2 + kk * 19 + n)
        torch.cuda.manual_seed(seed2 + kk * 19 + n)
        # Generate Higgs (P,Q)
        np.random.seed(seed=seed1 * (k+99) + n)
        ind1_surrogate = np.random.choice(N1_T, n, replace=False)
        np.random.seed(seed=seed2 * (k+101) + n)
        ind2_surrogate = np.random.choice(N2_T, n, replace=False)
        s1_surrogate = dataX[ind1_surrogate, :4]
        s2_surrogate = dataY[ind2_surrogate, :4]
    ...
    for k in range(N):
        ...
        if args.replace_P:
            np.random.seed(seed=seed2 * (k+299) + n)
            ind1 = np.random.choice(N1_T, n, replace=False)
            s1 = dataX[ind1, :4]
