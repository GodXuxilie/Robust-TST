...
def Wild_bootsrap_process(data_length, Num_trails, ln=0.5):
    # ln = 20.0
    #ln = 0.2 原参数0.2
    # ln = 5
    ar = np.exp(-1/ln)
...
def bootsrap_null(data_length, Num_trails, Kx, Ky, Kxy, alpha, device, dtype, ln):
    process = MatConvert(Wild_bootsrap_process(data_length, Num_trails, ln), device, dtype)
...
def compute_SCF_stat(X_org, Y_org, T_org, sigma0 ):
    """compute a deep kernel based SCF statistic."""
    ...
    Z = construct_z_theano(X_org,Y_org,T_org, sigma0)
    W = Z.mean(0)
    Sig = ((Z - W).transpose(1, 0)).mm((Z - W))
    if is_cuda:
        IJ = torch.eye(2*J).cuda()
    else:
        IJ = torch.eye(2*J)
    try:
...
def MMDu_WB(Fea, len_s, N_per, Fea_org, sigma, sigma0=0.1, epsilon = 10**(-10), is_smooth=True, is_var_computed=True, use_1sample_U=True, alpha=0.05,device=torch.device("cuda"), dtype=torch.float, ln=0.2):
    """compute value of deep-kernel MMD and std of deep-kernel MMD using merged data."""
    X = Fea[0:len_s, :] # fetch the sample 1 (features of deep networks)
    Y = Fea[len_s:, :] # fetch the sample 2 (features of deep networks)
    X_org = Fea_org[0:len_s, :] # fetch the original sample 1
    ...
    return bootsrap_null(len_s, N_per, Kx, Ky, Kxy, alpha, device, dtype, ln)


def TST_C2ST_S_MMD(S,N1,discriminator):
    ...
    return h1_mean_var_gram(Kx, Ky, Kxy, is_var_computed=True, use_1sample_U=True)

def TST_C2ST_L_MMD(S,N1,discriminator):
    """run C2ST-S on MNIST and CIFAR datasets."""
    np.random.seed(seed=1102)
    torch.manual_seed(1102)
    torch.cuda.manual_seed(1102)
...
def C2ST_NN_fit(S,y,N1,x_in,H,x_out,learning_rate_C2ST,N_epoch,batch_size,device,dtype):
    ...
    if is_cuda:
        ...
        model_C2ST = torch.nn.DataParallel(model_C2ST)
    else:
...
def construct_z_theano(Xth, Yth, Tth, gwidth_th):
        ...
        Xth = Xth/gwidth_th
        Yth = Yth/gwidth_th 
        # print(Xth.shape, Yth.shape, Tth.shape)
        # inverse Fourier transform (upto scaling) of the unit-width Gaussian kernel 
        fx = torch.exp((-(Xth**2).sum(1))/2).reshape((-1, 1))
        fy = torch.exp(((-(Yth**2).sum(1))/2)).reshape((-1, 1))
        # print(fx.shape, fy.shape)
        # n x J
        x_freq = Xth.mm(Tth.T)
        y_freq = Yth.mm(Tth.T)
        # zx: n x 2J
        zx = torch.cat([torch.sin(x_freq)*fx, torch.cos(x_freq)*fx], axis=1)
        zy = torch.cat([torch.sin(y_freq)*fy, torch.cos(y_freq)*fy], axis=1)
        z = zx-zy
        # print(z.shape)
        return z
...
def TST_MMD_adaptive_WB(Fea, N_per, N_te, sigma, sigma0, alpha, device, dtype, ln):
    ...
    return bootsrap_null(N_te, N_per, Kx, Ky, Kxy, alpha, device, dtype, ln)
...
def TST_WBMMD_u(Fea, N_per, N1, Fea_org, sigma, sigma0, epsilon, alpha, device, dtype, ln, is_smooth=True):
    """run two-sample test (TST) using deep kernel kernel."""
    ...
    return bootsrap_null(N1, N_per, Kx, Ky, Kxy, alpha, device, dtype, ln)
